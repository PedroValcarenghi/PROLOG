...
const logErrorQ = (msg) => (err) =>
    console.log(msg, err)
const logSuccessQ = (msg) => (data) =>
    console.log(`Success: `, data)

const queryM = (msg) => {
    console.log('msg teste =', msg.text),
        txt = msg.text.substring(2),
        console.log('msg txt =', txt),
        // Consulta 
        session.query(txt);
    //resposta
    bot.on('message', (msg) => session.answers(resp => bot.sendMessage(msg.chat.id, pl.format_answer(resp))));
    //Catch de Erros Ainda precisa ser implementada
    // .then(logSuccessQ(msg, match))
    // .catch(logErrorQ(`Error: `))
